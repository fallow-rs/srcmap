...
describe("generatedPositionFor", () => {
  ...
  it("throws for line < 1", () => {
    const map = new TraceMap(SIMPLE_MAP);
    assert.throws(() => generatedPositionFor(map, { source: "input.js", line: 0, column: 0 }));
    map.free();
  });
});
...
describe("allGeneratedPositionsFor", () => {
  ...
  it("returns empty array for unknown source", () => {
    const map = new TraceMap(SIMPLE_MAP);
    const positions = allGeneratedPositionsFor(map, {
      source: "nonexistent.js",
      line: 1,
      column: 0,
    });
...
describe("edge cases", () => {
  it("handles empty mappings", () => {
    ...
    assert.ok(Array.isArray(decoded));
    assert.equal(
      decoded.reduce((s, l) => s + l.length, 0),
      0,
    );
    map.free();
  });
