...
const { SourceMapGenerator: WasmGenerator } = require("@srcmap/generator-wasm");
...
export class SourceMapConsumer {
  ...
  generatedPositionFor(needle) {
    ...
    const result = this._wasm.generatedPositionForWithBias(
      resolvedSource,
      line - 1,
      column,
      wasmBias,
   ...
   */
  eachMapping(callback, context, _order) {
    const decoded = this._getDecodedMappings();
