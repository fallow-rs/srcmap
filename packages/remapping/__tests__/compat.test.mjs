...
import jrRemapping from '../../../benchmarks/node_modules/@jridgewell/remapping/dist/remapping.mjs'

// ── Helper ───────────────────────────────────────────────────────

const compareMaps = (srcmapResult, jrResult, label) => {
  assert.deepEqual(
    srcmapResult.sources,
    jrResult.sources,
    `${label}: sources mismatch`
  )
  assert.equal(
    srcmapResult.mappings,
    jrResult.mappings,
    `${label}: mappings mismatch`
  )
  assert.deepEqual(
    srcmapResult.names,
    jrResult.names,
