...
for (const { name, json } of maps) {
  ...
  for (
    let line = 0;
    line < srcmap.lineCount && checked < 100;
    line += Math.max(1, Math.floor(srcmap.lineCount / 50))
  ) {
    for (let col = 0; col < 200; col += 20) {
      ...
      if (expectedNull !== actualNull) {
        console.log(
          `  MISMATCH at ${line}:${col}: trace=${JSON.stringify(expected)}, srcmap=${JSON.stringify(actual)}`,
        );
        pass = false;
